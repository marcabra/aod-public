class AgenciesController < ApplicationController
  def index  
    if params[:organism].present?
      ...
      @grand_total = round @agencies.sum { |a| a.amount(current_year) }
      ...
      @organisms.each { |k, v| @organisms[k] = round(v) }
      @grand_total = @organisms.values.sum{ |v| v.to_i }
    else
      @departments = round Agency.by_kind('Administración General del Estado').by_year(current_year).sum("aids.paid_amount")
      @regions = round Agency.by_kind('Comunidades Autónomas').by_year(current_year).sum("aids.paid_amount")
      @town_halls = round Agency.by_kind('Entidades Locales').by_year(current_year).sum("aids.paid_amount")
      @universities = round Agency.by_kind('Universidades').by_year(current_year).sum("aids.paid_amount")
